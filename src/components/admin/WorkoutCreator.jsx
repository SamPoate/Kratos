...
const WorkoutCreator = ({ workoutData, setWorkoutData }) => {
    ...
    const onChange = workout => {
        ...
        workoutData['week_' + dict[week]]['day_' + dict[day]].forEach(
            (w, i) => {
        ...
        <section className='workout-creator'>
            ...
            <div className='form workout-creator__form' onSubmit={save}>
                ...
                <div className='save-add-buttons'>
                    ...
                    <button className='btn--white-text' onClick={getPhase}>
                        Get
                    </button>
                    <button
                        className='btn--white-text'
            ...
            {completed && <div className='completed-message'>{completed}</div>}
        </section>
    );
};
...
const FormGroup = ({ onChange, data, removeWorkoutGroup }) => {
    ...
    const [infoKey, setInfoKey] = useState(data.info_key || '');
    ...
    useEffect(() => {
        if (name || reps || sets || percent || weight || infoKey) {
            const workoutObj = {
                uuid: data.uuid,
                name: name || '',
                reps: reps || '',
    ...
    }, [name, reps, sets, percent, weight, infoKey]);
    ...
    return (
        <div className='workout-group'>
            <div className='workout-group__header'>
                <Icon
                    name='delete'
                    onClick={() => removeWorkoutGroup(data.uuid)}
                />
            ...
            <div className='form__input'>
                <label>Workout Information Key</label>
                <input
                    type='text'
                    name='infoKey'
                    onChange={e =>
