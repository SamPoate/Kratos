...
const Workout = props => {
    ...
    if (props.isLoaded && user) {
        ...
        if (props.profile.weights) {
            const dates = Object.keys(props.profile.weights).sort(
                (a, b) => a - b
            );
        ...
        return (
            <>
                <Table
                    ...
                    data={data}
                    profileWeights={
                        closestDate && props.profile.weights
...
const Table = ({
    ...
    const calcWeight = data => {
        ...
        const weight = (parseInt(profileWeights[workoutType]) / 100) * percent;
        data.weight = weight;
